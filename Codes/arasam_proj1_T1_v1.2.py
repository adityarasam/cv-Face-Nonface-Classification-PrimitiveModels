...
for i in range (0,10800):
    ...
    tmp_cov = np.cov(vector_3[i])
    
    Cov = np.append(Cov,tmp_cov)

Cov = np.array(Cov)    
cov_vis = np.diag(Cov)

cov_vis = np.sqrt(cov_vis)

cov_vis = np.diag(cov_vis)

cov_vis = np.array([cov_vis])
cov_vis = np.reshape(cov_vis, (60,60,3)).astype('uint8')


cv2.imshow('image2', mean_vis)
cv2.imshow('image3', cov_vis)
