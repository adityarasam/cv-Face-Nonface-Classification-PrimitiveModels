...
def  Initialize_GMM(NumOfModels, NumOfFeatures, NumOfImages):
    ...
    covar = np.random.rand(1,NumOfFeatures,NumOfModels)
...
if Task == 2:
    ...
    mean, covar, lmbda, r= Initialize_GMM(3, 75, 4)
    ...
    for i in range (1000,1004):
        ...
        covar1 = np.diag(((np.array([covar[0,:,0]]))).flatten())
        covar2 = np.diag(((np.array([covar[0,:,1]]))).flatten())
        covar3 = np.diag(((np.array([covar[0,:,2]]))).flatten())
